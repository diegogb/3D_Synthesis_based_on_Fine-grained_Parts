...
%%%%%
% This script allows you to replicate the same synthesized shapes (Chairs models) presented in Figure 13 of our paper:
...
%
% For example, the file "new_Fig13A-PC.ply" contains the synthesized point cloud of Figure 13(a), 
% and the file "new_Fig13A-Mesh.obj", contains the synthesized mesh of Figure 13(a)
%--------------------------------------------------------------------------
...
if (exist(mainDir, 'dir'))
    if (exist(fullfile(mainDir, 'synthBasedOnFG-Chairs.mat'), 'file'))
        ...
        templateIndices= fGetSegIndicesPerShape(segmentsArray, 22);
        
        fprintf('Generating shape of Figure 13(a)...\n');
        %%%
        %NOTE: 
        ...
        selSeg= selSegsFig13a;
        %%%
        
        %Synthesize the 3D point cloud of Figure 13(a)
        [err, ~, transforms] = fSynthesizeNovelShape(segmentsArray, samplingsArray, templateIndices, selSeg, segmentsAdj, fullfile(outdir, 'new_Fig13A-PC.ply'), true);
        %Synthesize the 3D mesh of Figure 13(a)
        if (~err)
            err= fSynthesizeMeshFromPoints(meshSegments, segmentsArray, samplingsArray, selSeg, transforms, templateIndices, segmentsAdj, true, fullfile(outdir, 'new_Fig13A-Mesh.obj'), false);
        end
        
        if (~err)
            %For figure 13(b)...
            fprintf('Generating shape of Figure 13(b)...\n');
            selSeg= selSegsFig13b;
            
            [err, ~, transforms] = fSynthesizeNovelShape(segmentsArray, samplingsArray, templateIndices, selSeg, segmentsAdj, fullfile(outdir, 'new_Fig13B-PC.ply'), true);
            if (~err)
                err= fSynthesizeMeshFromPoints(meshSegments, segmentsArray, samplingsArray, selSeg, transforms, templateIndices, segmentsAdj, true, fullfile(outdir, 'new_Fig13B-Mesh.obj'), false);
            end
        end
        
        if (~err)
            %For figure 13(c)...
            fprintf('Generating shape of Figure 13(c)...\n');            
            selSeg= selSegsFig13c;
            
            [err, ~, transforms] = fSynthesizeNovelShape(segmentsArray, samplingsArray, templateIndices, selSeg, segmentsAdj, fullfile(outdir, 'new_Fig13C-PC.ply'), true);
            if (~err)
                err= fSynthesizeMeshFromPoints(meshSegments, segmentsArray, samplingsArray, selSeg, transforms, templateIndices, segmentsAdj, true, fullfile(outdir, 'new_Fig13C-Mesh.obj'), false, false, true);
            end
        end
        
        if (~err)
            fprintf(['The process finished successfully! All the 3D Shapes of Figure 13 of the paper have been generated in the folder:\n' strrep(outdir,'\','\\')]);
            fprintf('\n');
        else
            fprintf('The script ended with errors. Some of the 3D shapes of Figure 13 of the paper probably were not generated\n');
        end
    else
