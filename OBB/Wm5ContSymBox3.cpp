...
SymBox3<Real>::SymBox3 (int numSamples, int numPoints, int numFaces, int numOrigFaces, const Vector3<Real>* samples, const Vector3<Real>* points, const Vector3<Real>* faces, const Vector3<Real>* origFaces, Real epsilon, Query::Type queryType)
{
	...
	if(hullFailed && tmpIndices != NULL)
		delete[] tmpIndices;
}

//****************************************************************************
//-----	-----	-----
//Changes by : Diego Gonzalez Becerra(*)
//(*)Prof.Oliver van Kaick's masters student at Carleton University. (2016-2017)
//-----
//This version of the template function "SymBox3" computes the Oriented Bounding Box 
//for a set of 3d points or samples (a point cloud(, without requiring any connectivity, 
//(i.e: this function does not requires a Triangular Mesh)
//-----
//-----	-----	-----
template <typename Real>
SymBox3<Real>::SymBox3(int numSamples, int numFaces, const Vector3<Real>* samples, const Vector3<Real>* faces, 
	Real epsilon, Query::Type queryType, bool doCalcSym)
{
	int i, j;
	Vector3<Real> origin, diff, U, V, W;
	Vector2<Real>* points2;
	...
	for (i = 0; i < hullQuantity; ++i)
	{
		...
		// Get 3D coordinate system relative to plane of triangle.
		origin = (samples[v0] + samples[v1] + samples[v2]) / (Real)3.0;
		Vector3<Real> edge1 = samples[v1] - samples[v0];
		Vector3<Real> edge2 = samples[v2] - samples[v0];
		W = edge2.UnitCross(edge1);  // inner-pointing normal
		if (W == Vector3<Real>::ZERO)
		{
			// The triangle is needle-like, so skip it.
		...
		//double pd = W.X() * points[v0].X() + W.Y() * points[v0].Y() + W.Z() * points[v0].Z();
		double pd = W.X() * samples[v0].X() + W.Y() * samples[v0].Y() + W.Z() * samples[v0].Z();
		pd = -1 * pd;
		Plane3<Real> facePlane(W, pd);
		...
		for (int rn = 0; rn < 5; rn++)
		{
			/*int ver = rand() % numPoints + 1;
			int side = facePlane.WhichSide(points[ver]);*/
			int ver = rand() % numSamples + 1;
			int side = facePlane.WhichSide(samples[ver]);
		...
		// Compute minimum area box in 2D.
		box2 = MinBox2<Real>((int)uniqueIndices.size(), points2, epsilon, queryType, false);
		...
		double* scores = new double[6];
		if (doCalcSym){
			scores = CheckSymPlanes(samples, numSamples, faces, numFaces, n1, center, n2, center, n3, center);
		}
		else {
			scores[3] = 100;
			scores[4] = 100;
		...
		*/
		...
		if (volume < minVolume)
		{
			...
			mSymBox.Axis[1] = box2.Axis[1].X()*U + box2.Axis[1].Y()*V;
			mSymBox.Axis[2] = W;
			mSymBox.Center = origin + box2.Center.X()*U + box2.Center.Y()*V + mSymBox.Extent[2] * W;
		}
	}
	...
	{
		//W = points[e2iter->V[1]] - points[e2iter->V[0]];
		W = samples[e2iter->V[1]] - samples[e2iter->V[0]];
		W.Normalize();

		std::set<EdgeKey>::const_iterator e1iter = e2iter;
		for (++e1iter; e1iter != edges.end(); e1iter++)
		{
			V = samples[e1iter->V[1]] - samples[e1iter->V[0]];
			V.Normalize();
			Real dot = V.Dot(W);
			if (Math<Real>::FAbs(dot) > Math<Real>::ZERO_TOLERANCE)
			{
			...
			for (++e0iter; e0iter != edges.end(); e0iter++)
			{
				U = samples[e0iter->V[1]] - samples[e0iter->V[0]];
				U.Normalize();
				dot = U.Dot(V);
				if (Math<Real>::FAbs(dot) > Math<Real>::ZERO_TOLERANCE)
				{
...
//-----
//This version of the function "CheckSymPlanes" is for a set of 3d points or samples (a point cloud), 
//without requiring any connectivity, (i.e: this function does not requires a Triangular Mesh).
//Instead, this function uses the faces of the Convex Hull of the set of points
//-----
template <typename Real>
double* SymBox3<Real>::CheckSymPlanes(const Vector3<Real>* samples, int ns, const Vector3<Real>* faces, int nf, 
	Vector3<Real>& normal1, Vector3<Real>& point1, Vector3<Real>& normal2, Vector3<Real>& point2, 
	Vector3<Real>& normal3, Vector3<Real>& point3)
{
	...
	for (int i = 0; i < ns; i++) {
		...
		for (int j = 0; j < nf; j++) {
			...
			Vector3<double> tp1(samples[v1].X(), samples[v1].Y(), samples[v1].Z());
			Vector3<double> tp2(samples[v2].X(), samples[v2].Y(), samples[v2].Z());
			Vector3<double> tp3(samples[v3].X(), samples[v3].Y(), samples[v3].Z());
...
//-----	-----	-----
//END Changes by : Diego Gonzalez Becerra
//-----	-----	-----
//****************************************************************************
