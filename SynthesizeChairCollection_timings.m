...
%%%%%
% This script allows you to generate a new collection of 3D shapes from our input set of Chairs, as well as a text file with timing statistics for
...
%   of the Chairs dataset. That is, "mainDir" is a folder that MUST contain the file: synthBasedOnFG-Chairs.mat
%   By default, this script assumes that the file synthBasedOnFG-Chairs.mat
%   is in the same folder of this script.
%   %%%
%   NOTE: if desired, you can copy the file synthBasedOnFG-Chairs.mat to any directory in your computer, 
...
%       1) a unique number for each Template Shape of the Chairs dataset
...
if (exist(mainDir, 'dir'))
    if (exist(fullfile(mainDir, 'synthBasedOnFG-Chairs.mat'), 'file')) && (exist(fullfile(mainDir, 'synthBasedOnFG-ChairsMS.mat'), 'file'))
        ...
        [err, ~, ~, ~, avgTimeSynthPC, avgTimeSynthMesh] = ...
            fSynthesizeNewShapeCollectionP(segmentsArray, samplingsArray, meshSegments, Dist, segmentsAdj, ...
            scoresArray, statsPerShape, taoProbThres, [outdir '\']);
        ...
        if (~err)
            ...
            try
                ...
                if (fid ~= -1)
                    ...
                    msgToP= ['Chairs set: synthesis of shape collection ending at:' curTstr];
                    ...
                    fprintf(['The process finished successfully! A new collection of 3D chair models is in the folder:\n' strrep(outdir,'\','\\')]);
                    fprintf('\n');
